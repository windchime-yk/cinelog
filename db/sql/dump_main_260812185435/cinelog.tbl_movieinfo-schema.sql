...
CREATE TABLE `tbl_movieinfo` (
  ...
  `is_subtitled` tinyint(1) NOT NULL COMMENT '字幕版かどうか',
  `is_domestic` tinyint(1) NOT NULL COMMENT '国内映画かどうか',
  `is_live_action` tinyint(1) NOT NULL COMMENT '実写かどうか',
  `theater_id` varchar(26) CHARACTER SET ascii COLLATE ascii_bin NOT NULL COMMENT '上映館テーブルID',
  `format_id` varchar(26) CHARACTER SET ascii COLLATE ascii_bin DEFAULT NULL COMMENT '鑑賞形式テーブルID',
  ...
  `companion_type_id` varchar(26) CHARACTER SET ascii COLLATE ascii_bin DEFAULT NULL COMMENT '同伴者分類テーブルID',
